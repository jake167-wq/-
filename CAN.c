...
uint8_t CAN1_mode_init(uint8_t tsjw, uint8_t tbs2, uint8_t tbs1, uint16_t brp, uint8_t mode)
{
    ...
    RCC_APB1PeriphResetCmd(RCC_APB1Periph_CAN1, ENABLE);  //使能复位
    RCC_APB1PeriphResetCmd(RCC_APB1Periph_CAN1, DISABLE); //失能复位
    ...
    CAN_InitStructure.CAN_ABOM = ENABLE;  //启动自动总线管理
    CAN_InitStructure.CAN_AWUM = DISABLE; //禁用自动唤醒
    CAN_InitStructure.CAN_NART = DISABLE; //禁用自动重传
    CAN_InitStructure.CAN_RFLM = DISABLE; //禁用 接收FIFO锁定模式
    CAN_InitStructure.CAN_TXFP = DISABLE; //禁用 传输FIFO优先级
    ...
    CAN_FilterInitStructure.CAN_FilterActivation = ENABLE; //启用筛选器
    CAN_FilterInit(&CAN_FilterInitStructure);

    CAN_ITConfig(CAN1, CAN_IT_FMP0, ENABLE); //使能接受中断
